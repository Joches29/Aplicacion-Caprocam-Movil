...
const esTablaValida = (tabla) => /^[a-zA-Z0-9_]+$/.test(String(tabla));

const ocultarValorSensible = (llave, valor) =>
  CAMPOS_SENSIBLES.includes(String(llave).toLowerCase()) && valor
    ? "********"
    : valor;
...
const limpiarRegistros = (registros) =>
  Array.isArray(registros)
    ? registros.map((registro) => limpiarRegistro(registro))
    : [];
...
export const obtenerTablasLocalesDebug = async () => {
  try {
    ...
    const resultado = tablas
      .map((item) => item.name)
      .filter((nombre) => !TABLAS_SISTEMA.includes(nombre))
      .filter((nombre) => !String(nombre).startsWith("sqlite_"));
    ...
    for (let i = 0; i < tablas.length; i += 1) {
      ...
      resumen.push({
        ...
        columnas: columnasRespuesta.success
          ? columnasRespuesta.data.map((columna) => columna.name)
          : [],
