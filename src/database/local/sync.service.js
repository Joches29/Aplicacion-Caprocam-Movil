 ...
 */
const enviarRegistroServidor = async (apiClient, endpoint, accion, registro) => {
    ...
    if (accion === "UPDATE") {
        ...
        if (!identificador) {
            throw new Error("No existe identificador para actualizar en servidor.");
        }
 ...
 */
export const descargarTablaLocal = async (apiClient, tabla) => {
    try {
        const endpoint = obtenerEndpointTabla(tabla);

        if (!endpoint) {
            return errorLocal("La tabla no tiene endpoint de sincronizacion.", tabla);
        }

        const respuestaHttp = await apiClient.get(endpoint);
        ...
        const respuestaGuardado = await localApi.sync.guardarDesdeServidor(tabla, registros);
 ...
 * @returns {Promise<object>} Respuesta local.
 */
export const descargarDatosInicialesLocal = async (apiClient) => {
    try {
        const resultados = [];
        ...
        for (const tabla of TABLAS_DESCARGA_INICIAL) {
            const resultado = await descargarTablaLocal(apiClient, tabla);
        ...
        return exitoLocal("Descarga inicial finalizada.", resultados);
    } catch (err) {
        return errorLocal("Error durante la descarga inicial.", err);
    }
};
...
export const sincronizarPendientesLocal = async (apiClient) => {
    try {
        ...
        return errorLocal("Error durante la sincronizacion de pendientes.", err);
    }
};
 ...
 * @returns {Promise<object>} Respuesta local.
 */
export const sincronizarCompletoLocal = async (apiClient) => {
    try {
        const subida = await sincronizarPendientesLocal(apiClient);
        const descarga = await descargarDatosInicialesLocal(apiClient);
