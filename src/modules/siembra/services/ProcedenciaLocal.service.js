...
async function ejecutarMetodoProcedencias(tipoMetodo, argumentos = []) {
    const apiProcedencias = localApi.procedencias;

    if (!apiProcedencias) {
        throw new Error("localApi.procedencias no esta disponible.");
    }
    ...
    for (let i = 0; i < nombres.length; i += 1) {
        ...
        if (typeof apiProcedencias[nombreMetodo] === "function") {
            return await apiProcedencias[nombreMetodo](...argumentos);
        }
    }

    throw new Error(`No existe metodo local para procedencias: ${tipoMetodo}`);
}
...
function mapearProcedenciaDesdeLocal(registro) {
    return registro
        ? {
...
async function mapearProcedenciaParaLocal(procedenciaDTO) {
    ...
    const nombre = obtenerValor(procedenciaDTO, ["nombre"], "");
    const descripcion = obtenerValor(procedenciaDTO, ["descripcion"], "");
...
async function getAll() {
    try {
        const respuesta = await ejecutarMetodoProcedencias("obtenerTodos");
        const data = obtenerDataRespuesta(respuesta);
        const registros = Array.isArray(data) ? data : [];

        return registros.map(mapearProcedenciaDesdeLocal).filter(Boolean);
    } catch (error) {
        console.error("Error al obtener procedencias locales", extraerError(error));
        throw error;
    }
}
...
async function getById(id) {
    try {
        const respuesta = await ejecutarMetodoProcedencias("obtenerPorId", [id]);

        return mapearProcedenciaDesdeLocal(obtenerDataRespuesta(respuesta));
    } catch (error) {
        console.error("Error al obtener la procedencia local", extraerError(error));
        throw error;
    }
}

async function create(procedenciaDTO) {
    try {
        // Compatibilidad: useNuevaSiembra llama a createProcedencia(nombre)
        // con un string suelto (ver services/procedencia.service.js), no
        // un DTO. Se acepta cualquiera de las dos formas.
        const datosOrigen =
            typeof procedenciaDTO === "string"
                ? { nombre: procedenciaDTO }
                : procedenciaDTO;

        const datosLocales = await mapearProcedenciaParaLocal(datosOrigen);
        const respuesta = await ejecutarMetodoProcedencias("crear", [datosLocales]);

        return mapearProcedenciaDesdeLocal(obtenerDataRespuesta(respuesta));
    } catch (error) {
        console.error("Error al crear la procedencia local", extraerError(error));
        throw error;
    }
}

async function update(id, procedenciaDTO) {
    try {
        const datosOrigen =
            typeof procedenciaDTO === "string"
                ? { nombre: procedenciaDTO }
        ...
        const datosLocales = await mapearProcedenciaParaLocal(datosOrigen);
        const respuesta = await ejecutarMetodoProcedencias("actualizar", [
            id,
            datosLocales,
        ]);
        ...
        console.error("Error al actualizar la procedencia local", extraerError(error));
        throw error;
    }
}
...
async function deleteById(id) {
    try {
        const respuesta = await ejecutarMetodoProcedencias("eliminar", [id]);

        return mapearProcedenciaDesdeLocal(obtenerDataRespuesta(respuesta));
    } catch (error) {
        console.error("Error al eliminar la procedencia local", extraerError(error));
        throw error;
    }
}
...
export default ProcedenciaLocalService;